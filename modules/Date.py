...
class Date:
    ...
    def random_anonymize(self, value, pattern):
        ...
        if "+-" in pattern or "-+" in pattern:
            ...
            temp_pattern = pattern.replace("-+", "")
            value_interval = re.findall(r"\((.*?)\)", temp_pattern)
        ...
        # Set a random value between range
            ...
            value_interval = re.findall(r"\((.*?)\)", pattern)[0].split(",")
            value_interval = [int(x) for x in value_interval]
            nvalue = random.randint(value_interval[0], value_interval[1])
    ...
    def check_conf_validity(self):
        ...
        if hasattr(conf.fields, self.field_name):
            if (
                not hasattr(getattr(conf.fields, self.field_name), "day")
                and hasattr(getattr(conf.fields, self.field_name), "month")
                and hasattr(getattr(conf.fields, self.field_name), "year")
            ):
                raise Exception(
    ...
    def anonymize(self, requirements):
        ...
        if "r" in getattr(conf.fields, self.field_name).day:
            ...
            self.day = self.random_anonymize(
                ref_day, getattr(conf.fields, self.field_name).day
            )
        ...
        if "r" in getattr(conf.fields, self.field_name).month:
            ...
            self.month = self.random_anonymize(
                ref_month, getattr(conf.fields, self.field_name).month
            )
        ...
        if "r" in getattr(conf.fields, self.field_name).year:
            self.year = self.random_anonymize(
                ref_year, getattr(conf.fields, self.field_name).year
            )
